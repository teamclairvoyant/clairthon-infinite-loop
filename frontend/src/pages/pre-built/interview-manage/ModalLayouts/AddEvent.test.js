...
import { mailIdOptions, dateOptions } from '../../../../common/MockData/mockTestData';
...
describe('AddEvent', () => {
  ...
  test('it test the input box value', () => {
    ...
    expect(endTimeInput.value).toStrictEqual(dayjs(dateHourLater).format('h:mm A'));
  ...
  test('it test the error message except title', async () => {
    ...
    const requiredErrorMessage = await screen.findAllByText(COPY.REQUIRED_ERROR_MESSAGE);
  ...
  test('it test the error message for field empty', async () => {
    ...
    const submitButton = screen.getByTestId(TEST_ID.COMMON.SUMBIT_BUTTON);
    fireEvent.click(submitButton);
    const requiredErrorMessage = screen.getAllByText(COPY.REQUIRED_ERROR_MESSAGE);
    expect(requiredErrorMessage.length).toEqual(3);

    const requiredErrorMessageDate = screen.getAllByText(COPY.REQUIRED_DATE_ERROR_MESSAGE);
    ...
    const requiredErrorMessageOnChange = screen.getAllByText(COPY.REQUIRED_ERROR_MESSAGE);
  ...
  test('it test the cancel anchor tag', async () => {
    ...
    const cancelAnchorTag = screen.getByTestId(TEST_ID.COMMON.ON_FORM_CANCEL_ANCHOR);
